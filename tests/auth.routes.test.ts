...
describe("Auth Router - Resend OTP integration tests", () => {
  ...
  describe("login mode resend", () => {
    ...
    it("creates new OTP for existing user and does not leak OTP in response", async () => {
      const mockLimit = vi.fn().mockResolvedValue([{ id: "user-1", emailVerified: true }]);
      const mockWhere = vi.fn(() => ({ limit: mockLimit }));
      const mockFrom = vi.fn(() => ({ where: mockWhere }));
      mockDb.select.mockImplementation(() => ({ from: mockFrom }));
  ...
  describe("register mode resend", () => {
    ...
    it("does not block resend for already-verified users (login flow)", async () => {
